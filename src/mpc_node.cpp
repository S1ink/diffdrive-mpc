...
private:
    ...
    void declareAndLoadParams()
    {
        ...
        params_.Q_ey = dbl("Q_ey", 200.0);
        params_.Q_ey_terminal = dbl("Q_ey_terminal", 400.0);
        params_.Q_eth = dbl("Q_eth", 10.0);
        params_.Q_eth_terminal = dbl("Q_eth_terminal", 50.0);
        params_.Q_v = dbl("Q_v", 5.0);
        params_.w_progress = dbl("w_progress", 0.2);

        params_.R_v = dbl("R_v", 0.5);
        params_.R_omega = dbl("R_omega", 0.5);
        params_.R_rate_v = dbl("R_rate_v", 2.0);
        params_.R_rate_omega = dbl("R_rate_omega", 5.0);

        params_.goal_threshold = dbl("goal_threshold", 0.3);
        params_.goal_cte_scale = dbl("goal_cte_scale", 2.0);
